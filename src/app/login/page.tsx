...
import { mockLoginUsers } from "@/mocks/users";
    ...
    <div className="min-h-screen bg-zinc-950 flex">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        ...
        >
          ...
          <div className="mb-6">
            <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">Acesso Rápido</p>
            <div className="grid grid-cols-2 gap-2">
              {mockLoginUsers.map((lu) => {
                const user = mockUsers.find((u) => u.id === lu.userId);
                return (
                  <button
                    key={lu.email}
                    onClick={() => fillCredentials(lu.email, lu.password)}
                    className="flex items-center gap-2.5 p-3 rounded-lg border border-zinc-700/50 bg-zinc-800/30 hover:bg-zinc-800/60 hover:border-zinc-600/50 transition-all text-left group"
                  >
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-500 to-indigo-500 flex items-center justify-center text-[10px] font-bold text-white shrink-0">
                      {user?.name.slice(0, 2).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-zinc-200 truncate">{user?.name.split(" ")[0]}</p>
                      <p className={cn("text-[10px] font-medium",
                        lu.role === "ADMIN" ? "text-violet-400" : "text-emerald-400"
                      )}>{lu.role}</p>
                    </div>
                  </button>
                );
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-zinc-950 px-3 text-zinc-600">ou entre manualmente</span>
            </div>
          </div>
          ...
          <p className="text-xs text-zinc-600 text-center mt-6">
            Ambiente de demonstração. Use os atalhos acima para acesso rápido.
          </p>
        </motion.div>
      </div>
    </div>
